...
Search uses agent_web.web_search (Tavily when keyed, else free DuckDuckGo) and
returns only REAL results — in live mode it never fabricates URLs; an empty scan
is reported honestly. Relevance ranking uses the heavy Gemini tier, with a
deterministic heuristic fallback so it never hangs.
"""
